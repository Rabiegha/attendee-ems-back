...
export class R2Service {
  ...
  /**
   * Liste tous les fichiers avec un préfixe donné
  ...
  /**
   * Supprime tous les badges de test (qui commencent par test-)
   * @returns Nombre de fichiers supprimés et erreurs éventuelles
   */
  async cleanupTestBadges(): Promise<{ deleted: number; errors: string[] }> {
    try {
      // Lister tous les fichiers dans le dossier badges/
      const allFiles = await this.listFiles('badges/');
      
      // Filtrer les fichiers de test
      const testFiles = allFiles.filter(key => 
        key.includes('/test-') || 
        key.includes('/nouveau-test-') || 
        key.includes('/test123')
      );

      let deleted = 0;
      const errors: string[] = [];

      // Supprimer chaque fichier de test
      for (const file of testFiles) {
        try {
          await this.deleteFile(file);
          deleted++;
          this.logger.log(`Deleted test file: ${file}`);
        } catch (error) {
      ...
      this.logger.error('Failed to cleanup test badges:', error);
